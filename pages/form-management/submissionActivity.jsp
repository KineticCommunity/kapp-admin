...
<!-- Show page content only if selected Kapp, Form, and Submission exists. -->
<c:choose>
    ...
    <c:when test="${empty submission}">
        <script>window.location.replace("${bundle.kappLocation}/${form.slug}?page=form-management/formActivity&kapp=${currentKapp.slug}&form=${currentForm.slug}");</script>
    </c:when>
    ...
    <c:otherwise>
        <bundle:layout page="${bundle.path}/layouts/layout.jsp">
            ...
            <bundle:variable name="breadcrumb">
                ...
                <li><a href="${bundle.kappLocation}/${form.slug}?page=form-management/formActivity&kapp=${currentKapp.slug}&form=${currentForm.slug}">${text.escape(currentForm.name)}</a></li>
            ...
            <div class="page-header form-submission">
                <h3>
            ...
            <div class="row form-activity m-b-4">
                ...
                <div class="col-xs-12 col-sm-6">
                    ...
                    by <c:choose>
                        <c:when test="${not empty teamsKapp}">
                            <a href="${bundle.spaceLocation}/${teamsKapp.slug}?page=user&username=${text.escapeUrlParameter(submission.createdBy)}">${submission.createdBy}</a>
                        </c:when>
                        <c:otherwise>
                            <span>${submission.createdBy}</span>
                        </c:otherwise>
                    </c:choose>
                </div>
                <div class="col-xs-12 col-sm-6">
                    ...
                    <c:choose>
                        <c:when test="${not empty submission.submittedAt}">
                            ...
                            by <c:choose>
                                <c:when test="${not empty teamsKapp}">
                                    <a href="${bundle.spaceLocation}/${teamsKapp.slug}?page=user&username=${text.escapeUrlParameter(submission.submittedBy)}">${submission.submittedBy}</a>
                                </c:when>
                                <c:otherwise>
                ...
                <div class="col-xs-12 col-sm-6">
                    ...
                    by <c:choose>
                        <c:when test="${not empty teamsKapp}">
                            <a href="${bundle.spaceLocation}/${teamsKapp.slug}?page=user&username=${text.escapeUrlParameter(submission.updatedBy)}">${submission.updatedBy}</a>
                        </c:when>
                        <c:otherwise>
                            <span>${submission.updatedBy}</span>
                        </c:otherwise>
                    </c:choose>
                </div>
                <div class="col-xs-12 col-sm-6">
                    ...
                    <c:choose>
                        <c:when test="${not empty submission.closedAt}">
                            ...
                            by <c:choose>
                                <c:when test="${not empty teamsKapp}">
                                    <a href="${bundle.spaceLocation}/${teamsKapp.slug}?page=user&username=${text.escapeUrlParameter(submission.closedBy)}">${submission.closedBy}</a>
                                </c:when>
                                <c:otherwise>
                                    <span>${submission.closedBy}</span>
                                </c:otherwise>
                            </c:choose>
                        </c:when>
                        <c:otherwise>
                            ...
                            <tbody>
                                <c:forEach var="descendant" items="${AdminHelper.formatDescendants(submission)}" varStatus="status">
                                    <tr>
                                        ...
                                        <td>
                                            <c:choose>
                                                <c:when test="${not empty teamsKapp && text.isNotBlank(descendant.submission.values['Assigned Individual'])}">
                                                    <a href="${bundle.spaceLocation}/${teamsKapp.slug}?page=user&username=${text.escapeUrlParameter(descendant.submission.values['Assigned Individual'])}">
