...
(function($, _, moment){
    ...
    typeAheadSearch = function(form){
        ...
        $(document).find('input[uses-typeahead]').each(function(){
            ...
            if(!_.isEmpty($(input).attr('typeahead-min-length'))) typeaheadConfig['minLength'] = parseInt($(input).attr('typeahead-min-length'))
...
typeaheadConfigurations = {
    defaultConfiguration:{
        ...
        placeholder: K.translate('bundle','Search by username...'),  // Placholder text to put into the search field
        ...
        selectedCallback: function(data, config) {  // Data is the row that was selected, all attributes returned from the bridge are available
            ...
            $.each( config['fieldsToSet'], function( key, value ) {
                ...
                $(config['typeaheadForm'].select('field[' + key + ']').element()).val(data[value]);
    ...
    personConfiguration:{
        ...
        attrsToShow: ['Username', 'Email', 'Display Name'],
        bridgedResource: 'Person - By Username',
        attrToSet: 'Username',
        fieldsToSet: {
            "Requested For":"Username",
            "Requested For Display Name":"Display Name"
        },
        selectedCallback: function(data, config){
            ...
            // Set the display to the name
            $('div.requested-for-display-name span.username').text(K('field[Requested For Display Name]').value());
            // Toggle edit and close
            $('div.requested-for-display-name span.fa').trigger('click');
